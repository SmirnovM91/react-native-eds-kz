                  ...
                  )
{
    @try{
        [self initialize];
        callback(@[[NSNull null], [self signPlain:certPath :certPassword :signData :@RSA_KEY]]);
    }
    @catch(NSException *exception){
        callback(@[exception.reason, [NSNull null]]);
    }
                  ...
                  )
{
    @try{
        [self initialize];
        callback(@[[NSNull null], [self signPlain:certPath :certPassword :signData :@AUTH_KEY]]);
    }
    @catch(NSException *exception){
        callback(@[exception.reason, [NSNull null]]);
    }
...
:(NSString*)keyType
{
    @try {
        ...
        X509* cert = [self validateCertificate:pkcs12_path :certPassword :keyType];        
        NSMutableDictionary *dictionary = [self writeBaseData:cert :signData];
        ...
        [dictionary setValue:signedXML forKey:@"signedXML"];
...
:(NSString*)keyType
{
    @try {
        ...
        unsigned char *cData = ( unsigned char *) [signData UTF8String];
        int cDataLen = strlen((char*)cData);
        ...
        mdCtx = EVP_MD_CTX_create();
        EVP_DigestInit_ex(mdCtx, md, NULL);
        EVP_DigestUpdate(mdCtx, cData, cDataLen);
        EVP_DigestFinal_ex(mdCtx, cHash, &cHashLen);
        EVP_MD_CTX_cleanup(mdCtx);
        ...
        unsigned int sigLen;
        EVP_SignInit_ex(mdCtx, md, NULL);
        EVP_SignUpdate (mdCtx, cData, cDataLen);
        EVP_SignFinal (mdCtx, cSignature, &sigLen, pkey);
        
  
        char *base64Signature = base64encode(cSignature, sigLen);
        
        NSData *signatureData = [NSData dataWithBytes:cSignature length:sigLen];
...
:(NSString*)keyType {
    ...
    const char *cPassword = ( char *) [password UTF8String];
    if (!PKCS12_parse(p12, cPassword, &pkey, &cert, &ca)) { //Error at parsing or password error
...
- (NSMutableDictionary *) writeBaseData
:(X509*)cert
:(NSString*)signData
{
    int len;
    ...
    id objects[] = { x509dictionary, certExpireDate, signData, certificate };
    id keys[] = { @"certData", @"certExpireDate", @"signedData", @"certificate" };
